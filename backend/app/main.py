...
class AgentRun(Base):
    ...
    status: Mapped[RunStatus] = mapped_column(SAEnum(RunStatus), default=RunStatus.CREATED, nullable=False)
...
class ToolRegistry:
    def __init__(self) -> None:
        self.tools: Dict[str, ToolSpec] = {
            ...
            "write_report": ToolSpec(
                name="write_report",
                description="Generate a structured report artifact. Requires approval before write-like behavior.",
                risk=ToolRisk.HIGH,
    ...
    def choose_tool(self, prompt: str) -> ToolSpec:
        ...
        if any(term in lowered for term in ["write", "create report", "save", "export", "delete", "send"]):
    ...
    def execute(self, tool: ToolSpec, prompt: str) -> Dict[str, Any]:
        if tool.name == "answer":
            return {
                "answer": "Agent run completed locally. Connect an LLM provider or document retriever next.",
...
app = FastAPI(title="First AI Agent - AgentOps Backend", version="0.1.0")
...
def audit(db: Session, event: str, payload: Dict[str, Any], run_id: Optional[str] = None) -> None:
    row = AuditLog(id=str(uuid.uuid4()), run_id=run_id, event=event, payload=payload)
    db.add(row)
...
def startup() -> None:
    init_db()
    logger.info("agentops backend started", extra={"database_url": DATABASE_URL})
...
def create_run(payload: RunCreate) -> RunResponse:
    ...
    with SessionLocal() as db:
        ...
        db.add(run)
        audit(db, "run_created", {"prompt_length": len(payload.prompt), "tool": tool.name}, run.id)

        if tool.requires_approval and not payload.dry_run:
            run.status = RunStatus.WAITING_APPROVAL
            audit(db, "approval_required", {"tool": tool.name, "risk": tool.risk.value}, run.id)
            db.commit()
            db.refresh(run)
            return serialize_run(run)

        if tool.requires_approval and payload.dry_run:
            run.status = RunStatus.WAITING_APPROVAL
            run.result = {"message": "Dry run selected a high-risk tool. Approval required before execution."}
            audit(db, "dry_run_approval_required", {"tool": tool.name}, run.id)
            db.commit()
            db.refresh(run)
            return serialize_run(run)
        ...
        try:
            ...
            db.add(call)
            audit(db, "tool_completed", {"tool": tool.name, "latency_ms": latency_ms}, run.id)
        except Exception as exc:
...
def approve_run(run_id: str, payload: ApprovalRequest) -> RunResponse:
    with SessionLocal() as db:
        ...
        if not payload.approved:
            ...
            run.updated_at = datetime.now(timezone.utc)
            audit(db, "approval_rejected", {"reason": payload.reason}, run.id)
            db.commit()
            db.refresh(run)
            return serialize_run(run)
        ...
        run.result = result
        run.updated_at = datetime.now(timezone.utc)
        audit(db, "approval_executed", {"tool": tool.name, "latency_ms": latency_ms}, run.id)
        db.commit()
        db.refresh(run)
        return serialize_run(run)
...
def smoke_evals() -> List[EvalResult]:
    checks = [
        EvalResult(name="tools_registered", passed=len(tool_registry.tools) >= 2, details="Tool registry has baseline tools."),
        EvalResult(name="approval_gate", passed=tool_registry.tools["write_report"].requires_approval, details="High-risk tool requires approval."),
        EvalResult(name="database_configured", passed=bool(DATABASE_URL), details="DATABASE_URL is configured."),
    ]
    return checks
